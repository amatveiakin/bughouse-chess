...
where
    ...
    F: for<'a> Fn(&'a mut T::Target),
{
    pub fn new(value: T, on_scope_end: F) -> Self { Self { value, on_scope_end } }
}
    ...
    F: for<'a> Fn(&'a mut T::Target),
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_tuple("Janitor").field(&self.value).finish()
    }
}
...
where
    ...
    F: for<'a> Fn(&'a mut T::Target),
{
    fn drop(&mut self) { (self.on_scope_end)(self.value.deref_mut()); }
}
